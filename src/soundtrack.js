...
export default class Soundtrack {
  constructor() {
    ...
    this.MIDI_NUM_NAMES = ["C_1", "C#_1", "D_1", "D#_1", "E_1", "F_1", "F#_1", "G_1", "G#_1", "A_1", "A#_1", "B_1",
  ...
  setGenerativeTrackStatus(_a, _b, _c) {
    ...
    if(arguments.length === 2) {
      ...
      if(arguments.length === 3) {
        ...
        console.log("[Soundtrack, setGenerativeTrackStatus] incorrect number of arguments: " + arguments.length);
        return;
      }
    }
    if(temp_status > 0) temp_status = true; else temp_status = false;
    switch(temp_trackNumber) {
      case 0: temp_holder.useSynth1Flag = temp_status; break;
      case 1: temp_holder.useSynth2Flag = temp_status; break;
      case 2: temp_holder.useSynth3Flag = temp_status; break;
      case 3: temp_holder.useSynth4Flag = temp_status; break;
      default: console.log('Soundtrack, setGenerativeTrackStatus, unhandled temp_trackNumber: ' + temp_trackNumber);
    }
  }
  ...
  */
  getGenerativeTrackStatusAsString(_a, _b) {
    ...
    if(arguments.length === 1) {
      ...
      if(arguments.length === 2) {
        ...
        console.log("[Soundtrack, getGenerativeTrackStatusAsString] incorrect number of arguments: " + arguments.length);
        return "error";
      }
    }
  ...
  */
  getGenerativeTrackStatus(_a, _b) {
    ...
    if(arguments.length === 1) {
      ...
      if(arguments.length === 2) {
        ...
        console.log("[Soundtrack, getGenerativeTrackStatus] incorrect number of arguments: " + arguments.length);
        return "error";
      }
    }
    if(isNaN(temp_trackNumber)) return "error";
    ...
    switch(temp_trackNumber) {
      case 0: return temp_holder.useSynth1Flag;
      case 1: return temp_holder.useSynth2Flag;
      case 2: return temp_holder.useSynth3Flag;
      case 3: return temp_holder.useSynth4Flag;
      default: console.log('Soundtrack, getGenerativeTrackStatus, unhandled temp_trackNumber: ' + temp_trackNumber);
    }
    return false;
  }

  transportPositionSegmentEquals(_part, _a, _b) {
    _part = _part.toLowerCase();
    switch(_part) {
      case 'bars':     if(_a.bars === _b.bars)         return true; else return false;
      case 'beats':    if(_a.beats === _b.beats)       return true; else return false;
      case 'quarters': if(_a.quarters === _b.quarters) return true; else return false;
      case 'ticks':    if(_a.ticks === _b.ticks)       return true; else return false; // if this value is represented by ticks...
      default: console.log('[transportEquals] unhandled _part = ' + _part);
    }
    return false;
  }

  copyTransportPosition(_src, _dst) {
    _dst.bars     = _src.bars;
  ...
  parseTransportPosition(_dst, _rawTransportData) {
  ...
  */
  midiToName(_holder, _midi) {
    ...
    if(_midi >= _holder.MIDI_NUM_NAMES) {
      console.log("[Soundtrack, midiToName] _midi >= _holder.MIDI_NUM_NAMES: " + _midi + " changed to _holder.MIDI_NUM_NAMES - 1" + " (" + _holder.MIDI_NUM_NAMES - 1 + ")");
  ...
  */
  ...
  /*
    Thanks to the _holder param this method may be used from callbacks.
  */
  randomChord(_holder, _numberOfNotes, _min, _max) {
    var result = [];
    if(isNaN(_numberOfNotes)) return result;
    _numberOfNotes = Math.abs(Math.floor(_numberOfNotes));
    if(_numberOfNotes < 1) return result;
  ...
  /*
    Thanks to the _holder param this method may be used from callbacks.
  */
  randomNote(_holder, _min, _max) {
    if(isNaN(_min)) _min = 0;
    if(isNaN(_max)) _max = 0;
    _min = Math.abs(Math.floor(_min));
    _max = Math.abs(Math.floor(_max));
    if(_min == _max) return _min;
    if(_min > _max) {var temp = _min; _min = _max; _max = temp;}
    var temp_range = _max - _min + 1;
    var result = Math.floor(Math.random() * temp_range) + _min;
    if(_holder.isNoteTuned(_holder, result) && (result <= _max) && (result >= _min))
      return result;
    var dir = 1;
    if(Math.random() < 0.5) dir = -1;
    if(result > _max) dir = -1; if(result < _min) dir = 1;
    var rep = 0; var max_rep = 300;
    while(rep < max_rep) {
      result += dir; if(result < 0) result = 127; if(result > 127) result = 0;
      if(_holder.isNoteTuned(_holder, result) && result >= _min && result <= _max)
        return result;
      rep += 1;
    }
    return result;
  ...
  init() {
    ...
    if(this.initializedFlag) {
      console.log("[Soundtrack, init] warning: attempt to reinitialize a Soundtrack object that has already been initialized");
    }
    else {
      ...
      this.synth2_freeverb = new Tone.Freeverb({"roomSize": 0.97, "dampening": 500, "wet": 1.0, "dry": 0.0});
      ...
      Tone.Transport.scheduleRepeat(function(_time) {
        ...
        */
        temp_holder.copyTransportPosition(temp_holder.current_transport_position, temp_holder.previous_transport_position);
        temp_holder.parseTransportPosition(temp_holder.current_transport_position, Tone.Transport.position);
        // ****************************** scale ****************************** \\
        if(
        ...
        if(
          ...
          if (
            ...
            temp_holder.synth2.triggerAttackRelease(temp_holder.mtof(temp_holder.randomNote(temp_holder, 56, 90)), "16n"); // temp_holder.mtof(temp_holder.randomNote(temp_holder, 60, 96))
          }
        }
        if(
          !temp_holder.transportPositionSegmentEquals(
          ...
          temp_holder.synth2.triggerAttackRelease(temp_holder.mtof(temp_holder.randomNote(temp_holder, 56, 90)), "16n"); 
        }
        // ***************************** synth 3 ***************************** \\
        if(
          !temp_holder.transportPositionSegmentEquals(
          ...
          if (
            ...
            temp_holder.synth3.triggerAttackRelease(temp_holder.mtof(temp_holder.randomNote(temp_holder, 30, 48)), "1n"); // temp_holder.mtof(temp_holder.randomNote(temp_holder, 60, 96))
          }
        }
        if(
          !temp_holder.transportPositionSegmentEquals(
          ...
          if (
            ...
            temp_holder.synth3_ghostNoteTriggeredFlag = false;
            temp_holder.synth3.triggerAttackRelease(temp_holder.mtof(temp_holder.randomNote(temp_holder, 30, 48)), "2n"); // temp_holder.mtof(temp_holder.randomNote(temp_holder, 60, 96))
          }
        }
        // ***************************** synth 4 ***************************** \\
        if(
          ...
          if (
            ...
            temp_holder.synth4.triggerAttackRelease(temp_holder.mtof(temp_holder.randomNote(temp_holder, 26, 46)), "1m"); // temp_holder.mtof(temp_holder.randomNote(temp_holder, 60, 96))
          }
        }
      }, "16n");
  ...
  /*
    Thanks to the temp_holder param this method may be used from callbacks.
  */
  generativePartVolume(_a, _b) { // _holder, _volume
    ...
    if(arguments.length === 1) {
      ...
      if(arguments.length === 2) {
        ...
        console.log("[Soundtrack, generativePartVolume] incorrect number of arguments: " + arguments.length);
      }
    }
  ...
  */
  ...
  /*
    Thanks to the temp_holder param this method may be used from callbacks.
  */
  effectVolume(_a, _b, _c) { // _holder, _effectNumber, _volume
  ...
  */
  _effectVolume(_holder, _effectNumber, _volume) {
    ...
    if(_effectNumber < 0) {
      console.log("[Soundtrack, _effectVolume] _effectNumber < 0: " + _effectNumber + " changed to 0");
      _effectNumber = 0; 
    }
    if(_effectNumber >= _holder.samples.length) {
      console.log("[Soundtrack, _effectVolume] _effectNumber >= this.samples.length: " + _effectNumber + " changed to this.samples.length - 1" + " (" + _holder.samples.length - 1 + ")");
  ...
  /*
    Thanks to the temp_holder param this method may be used from callbacks.
  */
  triggerEffect(_a, _b, _c) { // _holder, _effectNumber, _pitch
    ...
    if(arguments.length === 2) {
      ...
      if(arguments.length === 3) {
        ...
        console.log("[Soundtrack, triggerEffect] incorrect number of arguments: " + arguments.length);
      }
    }
  ...
  */
  _triggerEffect(_holder, _effectNumber, _normPitch) {
    ...
    if(_normPitch < 0.0) {
      console.log("[Soundtrack, _triggerEffect] _normPitch < 0.0: " + _normPitch + " changed to 0.0");
      _normPitch = 0.0; 
    }
    if(_normPitch > 1.0) {
      console.log("[Soundtrack, _triggerEffect] _normPitch > 1.0: " + _normPitch + " changed to 1.0");
      _normPitch = 1.0;
    }
    _effectNumber = Math.floor(_effectNumber);
    if(_effectNumber < 0) {
      console.log("[Soundtrack, _triggerEffect] _effectNumber < 0: " + _effectNumber + " changed to 0");
      _effectNumber = 0; 
    }
    if(_effectNumber >= _holder.samples.length) {
      console.log("[Soundtrack, _triggerEffect] _effectNumber >= this.samples.length: " + _effectNumber + " changed to this.samples.length - 1" + " (" + _holder.samples.length - 1 + ")");
      _effectNumber = _holder.samples.length - 1; 
    }
    ...
    else {console.log("[Soundtrack, _triggerEffect] samples not loaded (yet?)");}
  }
}